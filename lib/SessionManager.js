...
class SessionManager extends EventEmitter {
  ...
  _spawn (p) {
    ...
    ov.on('state',  (key, value, meta) => this._record('state', { username: p.username, snippetId: meta?.snippetId, key, value, source: meta?.source, exported: meta?.exported }))
  ...
  _snippetSummary (snippet) {
    const { code, ...summary } = snippet
    summary.codeLength = typeof code === 'string' ? code.length : 0
    return summary
  }

  _playerSummary (username, s) {
    const snippets = s.ov ? s.ov.inspect().map(snippet => this._snippetSummary(snippet)) : []
    return {
      username,
      status: s.status,
  ...
  list () {
    ...
    for (const [username, s] of this.sessions) {
      out.push(this._playerSummary(username, s))
    }
    return out
  }
  ...
  get (username) {
    ...
    if (!s) return null
    return this._playerSummary(username, s)
  }
  ...
  setState ({ targets, key, value }) {
    ...
    for (const username of resolved) {
      ...
      if (!s || !s.ov) { results.push({ username, ok: false, error: 'session not ready' }); continue }
      try {
        const stateEntry = s.ov._state.get(key)
        if (stateEntry?.schema) {
          s.ov.setExportedState(key, value)
        } else {
          s.ov.preset(key, value)
        }
        results.push({ username, ok: true })
      } catch (err) {
        results.push({ username, ok: false, error: err.message })
